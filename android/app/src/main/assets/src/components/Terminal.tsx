  ...
  const executeCommand = useCallback(async (cmd: string) => {
    ...
    switch (command) {
      ...
      case "python": case "python3": {
        ...
        addLine("info", "Para executar Python, use: Google Colab, ou instale Python localmente.");
      ...
      case "npm": {
        ...
        if (subCmd === "install" || subCmd === "i" || subCmd === "add") {
          ...
          if (pkgNames.length === 0) {
            addLine("info", "📦 npm install — instalando dependências do package.json...");
          } else {
            addLine("info", `📦 npm install ${pkgNames.join(" ")} — conectando ao servidor...`);
          }
          ...
          try {
            ...
            if (resp.ok) {
              ...
              for (const line of outLines.slice(0, 500)) {
                ...
                const display = isWarn  ? "⚠ " + line.replace(/^npm warn\s*/i, "")
                              : isError ? "✖ " + line.replace(/^(npm error|ERR!)\s*/i, "")
                              : line;
                addLine(type, display);
              }

              // Atualiza package.json com versões reais instaladas
              if (pkgNames.length > 0) {
              ...
              addLine("success", `✓ Instalação concluída!`);
              addLine("info", "  Nota: no editor browser, o require() é simulado. Para execução real, use o Terminal ▶");
            } else {
              throw new Error(`HTTP ${resp.status}`);
            }
          } catch (apiErr: any) {
            // Fallback: simulação offline
            addLine("warn", `Servidor indisponível (${apiErr.message}) — usando modo virtual...`);
            for (const pkgName of pkgNames) {
              const info = PACKAGES_DB[pkgName.toLowerCase()];
              const version = info?.version || "latest";
              ...
              addLine("success", `  ✓ ${pkgName}@${version} adicionado ao package.json`);
              if (info?.description) addLine("info", `    ${info.description}`);
            }
            addLine("success", pkgNames.length > 0
              ? `✓ ${pkgNames.length} pacote(s) adicionado(s) ao package.json`
              : "✓ package.json processado");
          }
