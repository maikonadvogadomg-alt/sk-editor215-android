...
export default function TemplateSelector({
  ...
  onOpenAssistenteJuridico, onOpenExtrator, onOpenScanner,
  ...
  const CREATE_OPTIONS = [
    ...
    },
    {
      ...
      label: "Clonar Repositório Git",
      desc: "Clonar um repositório do GitHub ou GitLab.",
      action: () => { setTab("tudo"); },
    },
    {
    ...
    <div className="h-screen bg-[#141c0d] flex flex-col text-white overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 pt-safe-top pt-4 pb-3 border-b border-gray-800/60 shrink-0">
        ...
        <div className="flex items-center gap-1">
          {onOpenExtrator && (
            <button onClick={onOpenExtrator} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-blue-800/20 border border-blue-600/30 text-blue-400 text-[11px] font-medium hover:bg-blue-800/30 transition-all" title="Extrator Jurídico">
              🔎 Extrator
            </button>
          )}
          {onOpenScanner && (
            <button onClick={onOpenScanner} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-sky-800/20 border border-sky-600/30 text-sky-400 text-[11px] font-medium hover:bg-sky-800/30 transition-all" title="Scanner de Dependências">
              🔍 Scanner
            </button>
          )}
          ...
          <>
            ...
            <div className="px-3 pt-3 pb-2 grid grid-cols-2 gap-2 border-b border-gray-800/40">
              <button onClick={onImportZip}
                ...
                <div>
                  <p className="text-[13px] font-bold text-yellow-300">Importar Arquivo</p>
                  <p className="text-[10px] text-gray-500">ZIP, TAR, ou qualquer formato</p>
                </div>
              </button>
