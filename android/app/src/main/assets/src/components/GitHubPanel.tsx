...
function MainPanel({
  ...
  if (view === "home") return (
    <div className="h-full flex flex-col">
      ...
      {status && (
        <div className={`mx-4 mb-2 px-3 py-2.5 rounded-xl border text-[12px] flex items-start gap-2 ${status.ok ? "bg-green-500/10 border-green-500/20 text-green-400" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
      ...
      {/* Botões Codespace/Gitpod após envio bem-sucedido */}
      {pushedRepo && status?.ok && (
        <div className="mx-4 mb-4 space-y-2">
          <p className="text-[10px] text-gray-600 uppercase tracking-wider font-semibold px-1">Abrir no IDE online</p>
          <a
            href={`https://codespaces.new/${pushedRepo.owner}/${pushedRepo.name}?quickstart=1`}
            target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-3 px-3 py-2.5 bg-[#1c2714] border border-green-500/30 rounded-xl hover:border-green-500/50 transition-colors"
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-bold text-green-400">Abrir no GitHub Codespace</p>
              <p className="text-[10px] text-gray-600">Terminal + npm install (precisa de login)</p>
            </div>
          </a>
          <a
            href={`https://gitpod.io/#https://github.com/${pushedRepo.owner}/${pushedRepo.name}`}
            target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-3 px-3 py-2.5 bg-[#1c2714] border border-gray-700/40 rounded-xl hover:border-gray-600/50 transition-colors"
          >
            <div className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center shrink-0">
              <ExternalLink size={13} className="text-orange-400" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-bold text-orange-400">Abrir no Gitpod</p>
              <p className="text-[10px] text-gray-600">IDE online com terminal (precisa de login)</p>
            </div>
          </a>
        </div>
      )}
