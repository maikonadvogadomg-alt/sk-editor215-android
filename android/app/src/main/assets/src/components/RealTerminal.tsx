...
export default function RealTerminal({
  ...
  const scheduleOutputFlush = (cmd: string) => {
    ...
    // Espera 6s de silêncio antes de considerar o comando encerrado
    // (garante captura de npm install, build, etc. com saídas em rajada)
    silenceTimerRef.current = setTimeout(() => {
      const out = outputBufferRef.current.slice(0, 64_000);
  ...
  useEffect(() => {
    ...
    // O terminal usa BASE_URL do Vite para que todas as chamadas passem
    // pelo proxy configurado (Vite → API server na porta 8080).
    const proto = window.location.protocol === "https:" ? "wss:" : "ws:";
    // Usa o BASE_URL do Vite para que a rota passe pelo proxy correto do editor
    const base = import.meta.env.BASE_URL?.replace(/\/$/, "") ?? "";
    const fallbackWsUrl = `${proto}//${window.location.host}${base}/api/ws/terminal`;
    ...
    fetch(`${base}/api/config`)
      ...
      .then((cfg) => {
        ...
        // Usa a URL do config, mas garante que passe pelo host/proxy do editor
        if (cfg?.terminalWsUrl) {
          // Sempre usa o host do editor para passar pelo proxy do Vite
          wsUrl = `${proto}//${window.location.host}${base}/api/ws/terminal`;
        }
    ...
    const connect = () => {
      ...
      ws.onmessage = (e: MessageEvent) => {
        ...
        // Buffer global aumentado para 50 mil chars (~3000 linhas de log)
        globalBufferRef.current = (globalBufferRef.current + plain).slice(-50_000);
        ...
        bufferFlushTimerRef.current = setTimeout(() => {
          onBufferUpdateRef.current?.(globalBufferRef.current, hasErr || ERROR_RE.test(globalBufferRef.current.slice(-2000)));
        }, 2000);
  ...
  function sendCmd(ws: WebSocket, term: Terminal | null, cmd: string) {
    ...
    runTimerRef.current = setTimeout(() => {
      ...
      if (currentCmdRef.current === cmd) {
        const out = outputBufferRef.current.slice(0, 64_000);
        onCommandOutput?.(cmd, out + "\n[Timeout — comando excedeu 19 minutos]", false);
        currentCmdRef.current = null;
        outputBufferRef.current = "";
      }
    }, CMD_TIMEOUT_MS);
