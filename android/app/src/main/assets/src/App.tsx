...
import AbrirOnline from "@/components/AbrirOnline";
...
function App() {
  ...
  // Botão Voltar do Android — fecha o painel aberto em vez de sair do app
  useEffect(() => {
    (window as any).androidBack = () => {
      if (showOnline)        { setShowOnline(false);        return true; }
      if (showBrowser)       { setShowBrowser(false);       return true; }
      if (showCampoLivre)    { setShowCampoLivre(false);    return true; }
      if (showAssistenteJur) { setShowAssistenteJur(false); return true; }
      if (showExtrator)      { setShowExtrator(false);      return true; }
      if (showScanner)       { setShowScanner(false);       return true; }
      if (currentProject)    { setCurrentProject(null);     return true; }
      return false;
    };
  }, [showOnline, showBrowser, showCampoLivre, showAssistenteJur, showExtrator, showScanner, currentProject]);
  ...
  if (showScanner) {
    return <Scanner onBack={() => setShowScanner(false)} />;
  }

  if (showBrowser) {
    return <InAppBrowser onBack={() => setShowBrowser(false)} />;
  }

  if (showOnline) {
    return <AbrirOnline files={currentProject ? vfs.toJSON() : {}} projectName={currentProject?.name ?? "projeto"} onBack={() => setShowOnline(false)} />;
  }
  ...
  return (
    <div style={filterStyle}>
      ...
      <EditorLayout
        ...
        onOpenOnline={() => setShowOnline(true)}
        onBackToProjects={() => {
          if (currentProject) {
